...
plot(filtered)
%using pan tompkins alg and finding out r peaks
y=length(x1);
fs = 200;              
N = length (x1);       
t = [0:N-1]/fs;        
...
if(heartrate<72)
disp('HEART RATE IS BELOW NORMAL ,IS SUFFERING FROM BRADYCARDIA');
else
disp('HEART RATE IS ABOVE NORMAL ,IS SUFFERING FROM TRACHYCARDIA');
end
